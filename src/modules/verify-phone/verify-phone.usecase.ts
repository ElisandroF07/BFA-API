...
// const accountSid = "ACc35b1bad545228d03424b2e18599a244"
// const authToken = "8495fef3f7a29e0fb308d6f5f08c039d"
// const messagingServiceSid = "MG54f7d5b3491c0fcd04cde5e3d23928f5"
...
export class VerifyPhoneUseCase {
    ...
    async sendOTP(phone_number: number, OTP: string): Promise<void> {
        await twilioClient.messages
        .create({
            body: `Olá! O seu código de verificação do BFA é ${OTP}. Não compartilhe este código com ninguém!`,
            messagingServiceSid: messagingServiceSid,
    ...
    async createRegister(phone_requester: number, response: Response): Promise<Response> {
        const existingPhone = await prismaClient.client_phones.findFirst({
            where: {
                phone_number: phone_requester
            },
            select: {
        ...
        try {
            await this.sendOTP(phone_requester, OTP);
            console.log('here1');
            
            let request = await prismaCient.register_requests.findFirst({
            ...
            await prismaClient.register_requests.upsert({

...
export class VerifyOTPUseCase {
    ...
    async createPhone(phone_requester: number, OTP: string, response: Response) {
        try {
            const phone = await prismaClient.register_requests.findFirst({
                where: { phone_number: phone_requester },
                select: { otp_code: true }
            });
            ...
            const result = await bcrypt.compare(OTP, phone.otp_code);
            ...
            const client = await prismaClient.client.findFirst({
                where: { client_phones: { some: { phone_number: phone_requester } } },
            ...
            if (client) {
                ...
                await prismaClient.client.create({
                    data: { role_id: 1, client_phones: { create: { phone_number: phone_requester, verified: true } } }
                });
            }

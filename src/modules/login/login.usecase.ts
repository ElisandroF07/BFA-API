...
export class LoginUseCase {
  ...
  async login(membership: string, access_code: string, response: Response) {
    ...
    if (membership.includes("@")) {
      ...
      const client_email = await prismaClient.client_email.findFirst({where: {email_address: membership.toLowerCase()}, select: {client_id: true}, cacheStrategy: { ttl: 1 }});
    ...
    if (client) {
      ...
      const email = await prismaClient.client_email.findFirst({where: {client_id: client.client_id}, select: {email_address: true}, cacheStrategy: { ttl: 1 }});
      // Verifica se a senha está correta
      if (await bcrypt.compare(access_code, client.access_code)) {
        // Retorna uma resposta de sucesso com o email e número de adesão do cliente

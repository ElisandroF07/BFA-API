...
export class GetCardDataUseCase{
  ...
  async getData(biNumber: string, response: Response){
    try {
      ...
      const card = await prismaClient.card.findFirst({where: {account_id: account?.account_id || 0}, select: {number: true, pin: true, role_id: true, created_at: true, nickname: true, state: true}})
